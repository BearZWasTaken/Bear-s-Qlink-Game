    ...
    : QMainWindow(parent)
{
    ...
    connect(twoPlayersGamePage, &TwoPlayersGamePage::ReturnToMenuSignal, this, [this]() {
        menuPage->Reset();
        stacked_widget->setCurrentWidget(menuPage);
    });
    ...
    connect(selectMapPage, &SelectMapPage::LoadMapSignal, this, [this](const QString &mapName, const int player_cnt) {
        if (player_cnt == 1)
        {
            stacked_widget->setCurrentWidget(onePlayerGamePage);
        ...
        {
            stacked_widget->setCurrentWidget(twoPlayersGamePage);
            twoPlayersGamePage->LoadGame(mapName);
        }
    });
